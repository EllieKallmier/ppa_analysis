...
def _get_daily_load_sums(df: pd.DataFrame) -> pd.DataFrame:
    ...
    the 'Load' values for each day.

    Args:
        df (pd.DataFrame): A DataFrame with a DateTime index and a 'Load' column
...
def _create_base_days(
    load_profile: pd.DataFrame, base_load_quantile: float
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Creates base load profiles for weekdays and weekends.

    This function calculates base load profiles for weekdays and weekends by
    grouping the load data by month and hour, and then computing the specified
    quantile (e.g., 0.5 for median) of the load values for each combination of
    hour in each month. It returns separate base load profiles for weekdays and
    weekends.

    Args:
        load_profile (pd.DataFrame): A DataFrame containing at least two columns:
            'Load' for the load values, and 'Weekend' (int) indicating whether the day
            is a weekend (1) or weekday (0).
        base_load_quantile (float): The quantile value (between 0 and 1) to calculate
            the base load (e.g., 0.5 for median load). This value is used to determine
            the level of inflexibility present in the load (i.e., how much load
            is not able to shift at all through each day).

    Returns:
        tuple: A tuple containing two DataFrames:
            - base_weekday (pd.DataFrame): The base load profile for weekdays,
              grouped by month and hour.
            - base_weekend (pd.DataFrame): The base load profile for weekends,
              grouped by month and hour.
    """
    ...
    base_weekday = (
        all_weekdays_only.groupby(
            [
                all_weekdays_only.index.month.rename("Month"),
                all_weekdays_only.index.hour.rename("Hour"),
            ]
        )["Load"]
        .quantile(base_load_quantile)
        .reset_index()
    )

    base_weekend = (
        all_weekends_only.groupby(
            [
                all_weekends_only.index.month.rename("Month"),
                all_weekends_only.index.hour.rename("Hour"),
            ]
        )["Load"]
        .quantile(base_load_quantile)
        .reset_index()
...
def daily_load_shifting(
    ...
    timeseries_data_ = timeseries_data.copy()
    timeseries_data_["DateTime"] = timeseries_data_.index.to_series() - timedelta(
        seconds=1
    )
    timeseries_data_ = timeseries_data_.set_index("DateTime", drop=True)

    timeseries_data_["Weekend"] = np.where(
        timeseries_data_.index.to_series().dt.weekday >= 5, 1, 0
    )
    ...
    daily_load_sums = _get_daily_load_sums(timeseries_data_)
    base_weekday, base_weekend = _create_base_days(timeseries_data_, base_load_quantile)
    all_time_max_load = timeseries_data_["Load"].max(numeric_only=True)

    # run optimisation for each day individually to keep constraints:
    for idx, date in enumerate(daily_load_sums.index):
        data_for_one_day = timeseries_data_[
            timeseries_data_.index.date == date.date()
        ].copy()
        ...
        if len(data_for_one_day) == 24:
            ...
            data_for_one_day["Flexible load"] = (
                data_for_one_day["Load"] - data_for_one_day["Base Load"]
            ).clip(lower=0.0)
            ...
            wholesale_prices = data_for_one_day["RRP"].clip(lower=0.0).values
            ...
            if sum(data_for_one_day["Flexible load"]) > 0:
                ...
                if advanced_settings.solver == "GUROBI":
                    ...
                    raise ValueError(
                        f"Solver name {advanced_settings.solver} not recognised."
                    )

                m = Model(solver_name=solver)

                load_dispatch = [
                    m.add_var(var_type=CONTINUOUS, lb=0.0, ub=all_time_max_load)
                    for i in len_day
                ]
                unmatched = [m.add_var(var_type=CONTINUOUS, lb=0.0) for i in len_day]
                raised_load = [
                    m.add_var(var_type=CONTINUOUS, lb=0.0, ub=all_time_max_load)
                    for i in len_day
                ]
                lowered_load = [
                    m.add_var(var_type=CONTINUOUS, lb=-1 * all_time_max_load, ub=0.0)
                    for i in len_day
                ]

                # Add 'ramp' constraints applied as a penalty term in the optimisation:
                ramp_up = [
                    m.add_var(var_type=CONTINUOUS, lb=0.0, ub=np.inf) for i in len_day
                ]
                ramp_down = [
                    m.add_var(var_type=CONTINUOUS, lb=-np.inf, ub=0.0) for i in len_day
                ]
                ...
                for i in len_day:
                    ...
                    m += (
                        unmatched[i]
                        >= (load_dispatch[i] + base_load[i]) - contracted_renewables[i]
                    )

                    # raised load is the positive difference between total load and original load:
                    m += (
                        raised_load[i]
                        >= (load_dispatch[i] + base_load[i]) - original_load[i]
                    )
                    m += (
                        lowered_load[i]
                        <= (load_dispatch[i] + base_load[i]) - original_load[i]
                ...
                for j in range(len(base_load) - 1):
                    m += ramp_up[j] >= (load_dispatch[j + 1] + base_load[j + 1]) - (
                        load_dispatch[j] + base_load[j]
                    )
                    m += ramp_down[j] <= (load_dispatch[j + 1] + base_load[j + 1]) - (
                        load_dispatch[j] + base_load[j]
                    )
                ...
                m += (
                    xsum((load_dispatch[i] + base_load[i]) for i in len_day)
                    >= load_sum_for_one_day
                )
                m += (
                    xsum((load_dispatch[i] + base_load[i]) for i in len_day)
                    <= load_sum_for_one_day
                ...
                if (
                    ...
                    day_result["Firm real"] = (
                        (day_result["Load dispatch"] + day_result["Base load"])
                        - day_result["Contracted Energy"]
                    ).clip(lower=0.0)
                    day_result["Firm check"] = round(
                        day_result["Firm real"], 3
                    ) == round(day_result["Firming"], 3)

                    if not day_result[~day_result["Firm check"]].empty:
                        print(day_result)
                        raise ValueError(
                            "wrong type of error atm but firming isn't right"
                        )
    ...
    timeseries_data = pd.concat(
        [timeseries_data, results_df["Load with flex"]], axis="columns"
    )

    return timeseries_data, results_df
