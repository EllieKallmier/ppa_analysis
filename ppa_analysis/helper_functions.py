...
from ppa_analysis.tariffs import (
    add_other_charges_to_tariff,
...
def check_leap_year(df: pd.DataFrame) -> bool:
...
def convert_gen_editor_to_dict(editor: dict) -> dict:
    """
...
def get_percentile_profile(
    period_str: str, data: pd.DataFrame, percentile: float
) -> pd.DataFrame:
    ...
    if period_str == "M":
        percentile_profile_period = data.groupby(
            [data.index.month.rename("Month"), data.index.hour.rename("Hour")]
        ).quantile(percentile)

    if period_str == "Q":
        percentile_profile_period = data.groupby(
            [data.index.quarter.rename("Quarter"), data.index.hour.rename("Hour")]
        ).quantile(percentile)
...
def yearly_indexation(
    ...
    df_with_strike_price["Strike Price (Indexed)"] = df_with_strike_price[
        "Strike Price (Indexed)"
    ].map(spi_map)
...
def quarterly_indexation(
    ...
    tuples = list(
        zip(
            df_with_strike_price.index.year.values,
            df_with_strike_price.index.quarter.values,
        )
    )
...
def get_data_years(cache_directory):
    ...
    year_counts = Counter(
        years_cache
    )  # Count the number of files in the cache for each year.
    # Get all the year that hav three files cached for each year.
    years_with_complete_data = [
        year for (year, count) in year_counts.items() if count >= 3
    ]
...
def calculate_lcoe(generator_info: dict[str:object]) -> float:
    ...
    first_capital_sum = (
        (
            capital
            * (1 + discount_rate) ** construction_years
            * discount_rate
            * (1 + discount_rate) ** economic_life
        )
        / (((1 + discount_rate) ** economic_life) - 1)
...
def get_seasons(load_and_gen: pd.DataFrame) -> pd.DataFrame:
    ...
    seasonal_load_and_gen.loc[
        seasonal_load_and_gen.index.month.isin([1, 2, 12]), "Season"
    ] = "Summer"
    seasonal_load_and_gen.loc[
        seasonal_load_and_gen.index.month.isin([3, 4, 5]), "Season"
    ] = "Autumn"
    seasonal_load_and_gen.loc[
        seasonal_load_and_gen.index.month.isin([6, 7, 8]), "Season"
    ] = "Winter"
    seasonal_load_and_gen.loc[
        seasonal_load_and_gen.index.month.isin([9, 10, 11]), "Season"
    ] = "Spring"
...
def get_weekends(load_and_gen: pd.DataFrame, region: str):
    ...
    holiday_dates = pd.Series(
        holidays.country_holidays(
            ...
            subdiv=region[:-1],
            years=range(
                weekend_load_and_gen.index.min().year,
                weekend_load_and_gen.index.max().year + 1,
            ),
        )
    )
    ...
    weekend_load_and_gen["Weekend"] = (
        weekend_load_and_gen["Date"].isin(holiday_dates.index)
...
def format_other_charges(extra_charges_collector: dict) -> dict:
    ...
    if len(extra_charges_collector) > 0:
        ...
        other_charges = {
            "Energy Charges": {
                "Peak Rate": {"Unit": "$/kWh", "Value": 0.0},
                "Shoulder Rate": {"Unit": "$/kWh", "Value": 0.0},
                "Off-Peak Rate": {"Unit": "$/kWh", "Value": 0.0},
                "Retailer Demand Charge": {"Unit": "$/kVA/day", "Value": 0.0},
            },
            "Metering Charges": {
                "Meter Provider/Data Agent Charges": {"Unit": "$/Day", "Value": 0.0},
                "Other Meter Charges": {"Unit": "$/Day", "Value": 0.0},
            },
            "Environmental Charges": {
                "LREC Charge": {"Unit": "$/kWh", "Value": 0.0},
                "SREC Charge": {"Unit": "$/kWh", "Value": 0.0},
                "State Environment Charge": {"Unit": "$/kWh", "Value": 0.0},
            },
            "AEMO Market Charges": {
                "AEMO Participant Charge": {"Unit": "$/kWh", "Value": 0.0},
                "AEMO Ancillary Services Charge": {"Unit": "$/kWh", "Value": 0.0},
            },
            "Other Variable Charges": {
                "Other Variable Charge 1": {"Unit": "$/kWh", "Value": ""},
                "Other Variable Charge 2": {"Unit": "$/kWh", "Value": ""},
                "Other Variable Charge 3": {"Unit": "$/kWh", "Value": ""},
            },
            "Other Fixed Charges": {
                "Total GST": {"Unit": "$/Bill", "Value": ""},
                "Other Fixed Charge 1": {"Unit": "$/Bill", "Value": ""},
                "Other Fixed Charge 2": {"Unit": "$/Bill", "Value": ""},
            },
        }
...
def get_load_data_chunk(
    load_data: pd.DataFrame, end_date: pd.Timestamp
...
def get_selected_tariff(
    chosen_tariff: dict | str, extra_charges_collector: dict, retail: bool = False
) -> dict:
    """
