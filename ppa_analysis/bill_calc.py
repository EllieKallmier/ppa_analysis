...
def calculate_ppa(
    ...
    indexation: float
    | list[float] = 0,  # set a default for no indexation, for simplicity
    ...
    indexation_calc = {
        "Y": helper_functions.yearly_indexation,
        "Q": helper_functions.quarterly_indexation,
    }
    strike_prices_indexed = indexation_calc[index_period](
        price_and_load, strike_price, indexation
    )
    ...
    price_and_load["Price"] = price_and_load["Strike Price (Indexed)"] - np.maximum(
        price_and_load["RRP"], floor_price
    )
    price_and_load["Wholesale Cost"] = (
        price_and_load["Contracted Energy"] * price_and_load["RRP"]
    )
    price_and_load["PPA Settlement"] = price_and_load["Contracted Energy"] * (
        price_and_load["Strike Price (Indexed)"]
        - np.maximum(price_and_load["RRP"], floor_price)
    )

    price_and_load["PPA Value"] = (
        price_and_load["Strike Price (Indexed)"] * price_and_load["Contracted Energy"]
    )
    price_and_load["PPA Final Cost"] = price_and_load[
        ["Wholesale Cost", "PPA Settlement"]
    ].sum(axis=1)
...
def calculate_tariff_bill(
    ...
    """
    resampled_load_index = (
        load_and_gen_data.copy().resample(settlement_period).sum(numeric_only=True)
    )
    resampled_load_index = resampled_load_index.index

    # Load in all the tariffs and find the selected tariff (network charges):
    tariff_bill_results = pd.DataFrame(
        index=resampled_load_index, columns=[f"{bill_type} Bill ($)"]
    )
    ...
    for end_date in resampled_load_index:
        load_chunk, remainder = helper_functions.get_load_data_chunk(
            remainder, end_date
        )
        ...
        network_load = (
            load_chunk[["DateTime", col_name_to_calc]]
            .copy()
            .rename(columns={"DateTime": "TS", col_name_to_calc: "kWh"})
        )
        network_bill = tariffs.tariff_bill_calculator(network_load, selected_tariff)
        tariff_bill_results.loc[end_date, f"{bill_type} Bill ($)"] = float(
            network_bill["Retailer"]["Bill"][0]
        )
...
def calculate_firming(
    ...
    firming_costs["Unmatched Energy"] = (
        firming_costs["Load"] - firming_costs["Contracted Energy"]
    ).clip(lower=0.0)

    if firming_type == "Retail":
        firming_costs = calculate_tariff_bill(
            firming_costs, settlement_period, tariff_details, "Retail"
        )
        firming_costs = firming_costs.rename(
            columns={"Retail Bill ($)": "Firming Costs"}
        )
    else:
        firming_costs["Firming Costs"] = (
            firming_costs["Unmatched Energy"] * firming_costs["Firming price"]
        )
...
def calculate_excess_electricity(
    ...
    if isinstance(excess_price, str) and excess_price != "Wholesale":
        raise ValueError(
            'excess_price should be one of "Wholesale" or a float representing the fixed on-sell price.'
        )
    ...
    load_contracted_volume["Excess Energy"] = (
        load_contracted_volume["Contracted Energy"] - load_contracted_volume["Load"]
    ).clip(lower=0.0)
    load_contracted_volume["Excess Energy Revenue"] = (
        load_contracted_volume["Excess Energy"] * load_contracted_volume["Excess Price"]
    )

    resampled = (
        load_contracted_volume.resample(settlement_period).sum(numeric_only=True).copy()
    )
...
def calculate_shortfall(
    ...
    if contract_type in ["Pay as Produced", "Pay as Consumed"]:
        ...
        df_resamp["Shortfall"] = (
            df_resamp["Guaranteed Energy"] - df_resamp["Contracted Energy"]
        ).clip(lower=0.0)
        df_resamp["Shortfall"] *= shortfall_penalty

    elif contract_type in ["Baseload", "Shaped"]:
        volume["Shortfall"] = (volume["Contracted Energy"] - volume["Hybrid"]).clip(
            lower=0.0
        )
        volume["Shortfall"] *= (
            shortfall_penalty  # IF the seller contracts other "Replacement Energy" - set shortfall penalty to zero.
        )
        df_resamp = volume.resample(settlement_period).sum(numeric_only=True).copy()
        ...
        df_resamp_247["Match %"] = np.where(
            df_resamp_247["Load"] == 0,
            100,
            np.minimum(
                df_resamp_247["Contracted Energy"] / df_resamp_247["Load"] * 100, 100
            ),
        )
        df_resamp_247 = df_resamp_247.resample(settlement_period).mean(
            numeric_only=True
        )
        df_resamp_247["Shortfall %"] = (
            guaranteed_percent - df_resamp_247["Match %"]
        ).clip(lower=0.0)

        df_resamp = pd.concat([df_resamp, df_resamp_247["Shortfall %"]], axis="columns")
        df_resamp["Shortfall"] = (
            df_resamp["Load"] * df_resamp["Shortfall %"] / 100 * shortfall_penalty
        )
...
def calculate_lgcs(
    ...
    df_to_check["Volume Difference"] = (
        df_to_check["Load"] * (guaranteed_percent / 100)
    ...
    df_to_check["LGC Undersupply"] = np.where(
        df_to_check["Volume Difference"] > 0, df_to_check["Volume Difference"], 0.0
    )
    df_to_check["LGC Oversupply"] = np.where(
        df_to_check["Volume Difference"] < 0, df_to_check["Volume Difference"], 0.0
    )
...
def calculate_wholesale_bill(
    ...
    wholesale_bill = (
        data[["Load", "Wholesale Cost"]]
        .resample(settlement_period)
        .sum(numeric_only=True)
    )

    wholesale_bill["LGC Cost"] = wholesale_bill["Load"] * lgc_buy_price
    wholesale_bill["Total"] = (
        wholesale_bill["LGC Cost"] + wholesale_bill["Wholesale Cost"]
    )
...
def calculate_bill(
    ...
    # 2. Firming costs:
    firming_costs = calculate_firming(
        volume_and_price, firming_type, tariff_details, settlement_period
    )
    results["Firming Costs"] = firming_costs["Firming Costs"].copy()

    # 3. Network costs:
    network_costs = calculate_tariff_bill(
        volume_and_price, settlement_period, tariff_details, "Network"
    )
    results["Network Costs"] = network_costs["Network Bill ($)"].copy()

    # 4. Revenue from on-sold excess RE:
    excess_val = calculate_excess_electricity(
        volume_and_price, settlement_period, excess_price
    )
    results["Revenue from on-sold RE"] = -1 * excess_val["Excess Energy Revenue"].copy()

    # 5. and 6. Revenue/Cost of LGCs
    lgc_balance = calculate_lgcs(
        volume_and_price,
        settlement_period,
        lgc_buy_price,
        lgc_sell_price,
    ...
    shortfall_payment_received = calculate_shortfall(
        volume_and_price,
        settlement_period,
        contract_type,
        shortfall_penalty,
    ...
    results["Shortfall Payments Received"] = (
        -1 * shortfall_payment_received["Shortfall"]
    )
