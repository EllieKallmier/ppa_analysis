...
def calc_by_row(row, price_profile, residual_profiles):
    """
    Calculate the PPA cost by calling the the calc function and selecting the required data from the scenario row.
    ...
    cost = calc(
        ...
        lgc_volume=row['LGC_Purchase_Volume'], 
        lgc_price=row['LGC_Purchase_Price'], 
        load_mlf=row['Load_MLF'],
        load_dlf=row['Load_DLF'], 
        gen_mlf=row['Generator_MLF'], 
        gen_dlf=row['Generator_DLF'],
        penalty_period=row['Target_Period'], 
        yearly_target_volume_mwh=row['Yearly_Target_MWh'],
        penalty_rate_mwh=row['Yearly_Short_Fall_Penalty_MWh'], 
        yearly_target_volume_lgc=row['Yearly_LGC_target_LGC'],
        penalty_rate_lgc=row['Yearly_LGC_short_fall_penalty_LGC'], 
